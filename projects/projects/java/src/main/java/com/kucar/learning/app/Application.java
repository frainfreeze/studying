...
public class Application extends JFrame {
  ...
  private JComponent createListPane() {
    ...
    //listener, this method gets invoked every time user makes selection
  ...
  private void refreshData() {
    ...
    //swing worker allows us to run possibly long task without freezing the gui app
    final SwingWorker<Void, Contact> worker = new SwingWorker<Void, Contact>() {
      ...
      protected Void doInBackground() throws Exception {
        ...
        for (final Contact contact : contacts) {
        	//copies the contact object from the swing worker thread to the event dispatcher thread
        	publish(contact);
        }
        return null;
      }
      ...
      //executed on event dispatcher thread
      @Override
  ...
  private void save() {
	  //TODO: put save into swing worker?
    if (selected != null) {
      try {
