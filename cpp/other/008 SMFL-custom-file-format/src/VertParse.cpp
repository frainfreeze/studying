#include "VertParse.h"
#include <iostream> // remove me after testing
...
void VertParser::Parse(){
    ...
    while (std::getline(s, line)){
        ...
        while (std::getline(ss, segment, '=') && std::getline(ss, segment2)){
            ...
            std::cout << segment << ", " << segment2 << std::endl; // remove me after testing

            if (segment=="quadnum") quadCotainer.numQuads = std::stoi(segment2);
            if (segment.find("quad.") == 0) quadCounter++;

            switch(quadCounter){
                case 1: foo.a.x = std::stoi(segment2); break;
                case 2: foo.a.y = std::stoi(segment2); break;
                case 3: foo.b.x = std::stoi(segment2); break;
                case 4: foo.b.y = std::stoi(segment2); break;
                case 5: foo.c.x = std::stoi(segment2); break;
                case 6: foo.c.y = std::stoi(segment2); break;
                case 7: foo.d.x = std::stoi(segment2); break;
                case 8: foo.d.y = std::stoi(segment2); break;
            default: break;
            }
        }
        if (quadCounter >= 8){
